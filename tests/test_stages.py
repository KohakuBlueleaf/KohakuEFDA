...
def test_stage_order_and_parameter_defaults() -> None:
    ...
    params = params_of("layout", {"spread_attempts": "200", "seed": 3})
    assert params["spread_attempts"] == 200 and params["seed"] == 3
...
def test_layout_stage_records_frames_and_a_checkpoint(
    ...
    placement, layout = layout_stage(
        dataset,
        built,
        {"spread_attempts": 2000, "frame_every": 10},
        frames.append,
    )
    catalogue = frames[0]
...
def test_layout_is_reproducible_for_a_seed(dataset: Dataset, netlist) -> None:
    _, built = netlist
    settings = {"spread_attempts": 2000, "seed": 5}
...
def test_cancellation_stops_the_layout(dataset: Dataset, netlist) -> None:
    ...
    with pytest.raises(CancelledError):
        layout_stage(dataset, built, {"spread_attempts": 2000}, None, lambda: True)
