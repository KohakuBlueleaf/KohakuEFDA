...
| `moves.py` | `ConstructionMoves` (regional and local repair regions), `LayoutMoves` (shift, rotate, swap, cluster, reroute, cut, pull, repack), the `MOVES` names |
| `compact.py` | `relocate`, `cut_candidates`, `CompactionMoves` (cut, pull) |
| `repack.py` | `RepackMoves`: a neighbourhood withdrawn and reconnected at ranked anchors inside one attempt |
