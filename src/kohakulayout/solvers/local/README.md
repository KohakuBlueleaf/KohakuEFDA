...
| `search.py` | `Trajectory`: construction by the regional repair of the given `search` accepted on the gap delta, improvement by moves accepted on the area-first delta; the next move starts from current |
