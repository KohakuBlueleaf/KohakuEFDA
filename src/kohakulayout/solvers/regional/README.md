...
| `search.py` | `Search`: trials that restart or restore the best prefix and withdraw a region, insert by priority with pressure (pinned cells first, then cells with a placed neighbour) through a routed lookahead (`lookahead` placed anchors compared by wire cells, three by default; `insert_failures` refusals end the scan), refill, retain the best by placed cells; `defaults`, `proposer`, `neighbourhood` (which cells count as neighbours: every cell sharing a net by default) and `priority` are what a project's construction overrides; `neighbours_of`, `clear`; every retained best is offered to the context, so a budget that ends mid-trial still returns it |
