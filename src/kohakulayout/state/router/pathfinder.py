...
def entry(search: Search, xy: XY, direction: XY | None) -> Entry | None:
    ...
    if unit_ref is not None:
        ...
        if rule.mode == "unit" and (
            rule.unit is None
            or not world.free_footprint(rule.unit, xy[0], xy[1], 0)
            and not _only_wires(holders)
        ):
            return None
    return result
...
def find(
    ...
    """The cheapest path from any source to any target; None when none exists within the step budget."""
...
__all__ = ["DIRS", "Found", "Search", "entry", "find", "straight_through"]
