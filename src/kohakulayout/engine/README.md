...
| file | what |
|---|---|
| `context.py` | `Context`: problem, physics, world, seeded rng, budget, plugins, progress sink, `builder()`, `attempt(fn)`, `snapshot`/`restore`, `assess`, `consider`/`accept`/`discard`, `frame`, `checkpoint`/`resume`, `gather`, `scope` |
