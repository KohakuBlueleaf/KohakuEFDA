...
| `fields` | 2, fields | `needs(cell)`, `emitters()` (kind, footprint, reach, overlap rule), `cover(world, needs)`, `sweep(kind)` (the kind's emitters laid afresh from every placed need at once, after every placement; a swept emitter stands wherever nothing holds its cells) | no fields; `GreedyCover` and `KindCover` ship as planners |
| `boundaries` | 3, boundaries | `anchors(world, cell)` for a constraint kind, `legal(world, placement)`, `outside(world, net)`, `crossing_region(carrier, region)`, `unit_region(unit, region)` (where a crossing or junction unit may stand) | every anchor in `build`; nothing extra illegal; units anywhere |
