...
| `test_layout.py`, `test_importer.py` | Hand-built lines through the reverse translation: connections, the framework's findings and the pack's deck, the routed evaluation, splitters, conduits, dumps, direct links; the IndustrialPlanner importer on a small fixture and the `check` command. |
| `test_layout_promises.py` | The layout stage on hand-built netlists: wired machines end close, routed and powered; the parked core stands inside the area and counts; outside inputs on the border feed a zone; a laid bus seats its bricks and stays one cluster; Valley IV bricks take slots; the core's ports carry supply and delivery; cancellation and a budget that cannot run. |
...
| `test_place_route.py` | Two brick pairs on a Wuling bus route through the stage and deliver both items; the three benchmarks lay out clean and at the plan's rates inside their area with the core placed and modules within blueprint limits; the `layout` command writes its artifacts. |
